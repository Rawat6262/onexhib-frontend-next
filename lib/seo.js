...
export const PUBLIC_ROUTES = Object.freeze({
  ...
  // Hub for the country and city landing pages. "exhibitions-in" rather than
  // "locations" because the URL then reads as the query people actually type:
  // /exhibitions-in/germany/berlin. See lib/locations.js for the quality
  // threshold that decides which places get a page at all.
  locations: "/exhibitions-in",
  // Hub for the industry landing pages: /exhibitions-for/technology. Mirrors
  // the "exhibitions-in" convention — "in" for a place, "for" for an industry —
  // and avoids /exhibitions/[slug], which the authenticated app already owns.
  categories: "/exhibitions-for",
  companies: "/companies",
  products: "/products",
  services: "/exhibition-services",
  // Editorial content. Unlike every other route here, this one is noindex
  // while it has no posts — an empty hub is a thin page, and the sitemap
  // omits it too. See lib/blog.js and app/(public)/blog/page.jsx.
  blog: "/blog",
});
